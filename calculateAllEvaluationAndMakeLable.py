...
parser.add_argument('-o', '--the_output', help='the output maked lable file, eg: /mnt/X500/farmers/wangshj/doctoralPeriod/thirdPoint/simData/markedLableFile.tsv', required=True, type=str)
args = parser.parse_args()
batchSamlesDir = args.the_input
makedLableFileName = args.the_output
...
resultFile = open(makedLableFileName, 'w')
for one_faFeature in faFeature_arr:
    ...
    for one_readLen in readLen_arr:
        ...
        for one_depth in depth_arr:
            ...
            maxRecall = 0
            maxF_measure = 0
            for one_software in software_arr:
                vcfFile = batchSamlesDir + "/" + one_faFeature + "/testAlterDepth_" + one_readLen + "readLen/" + one_depth + "/" + one_software + "/" + one_software + ".vcf"
                predictionArr = getPredictionArr(vcfFile)
                answerPlusPredictionArr = getAnswerPlusPredictionArr(predictionArr, answerArr)
                tp, fp, tn, fn, accuracy, precision, recall, f_measure = caculate(answerArr, predictionArr, answerPlusPredictionArr)
                if precision >= maxPrecision:
                    maxPrecision = precision
                    maxPrecisionSoftware = one_software
                if recall >= maxRecall:
                    maxRecall = recall
                    maxRecallSoftware = one_software
                if f_measure >= maxF_measure:
                    maxF_measure = f_measure
                    maxF_measureSoftware = one_software
            #resultFile_row = faFeature_dic[one_faFeature][0] + "\t" + faFeature_dic[one_faFeature][1] + "\t" + faFeature_dic[one_faFeature][2] + "\t" + faFeature_dic[one_faFeature][3] + "\t" + RMB + "\t" + highMutationDensePercent + "\t" + one_readLen + "\t" + one_depth + "\t" + maxPrecisionSoftware + "\t" + maxRecallSoftware + "\t" + maxF_measureSoftware + "\n"      #输出为tsv格式
            resultFile_row = faFeature_dic[one_faFeature][0] + "," + faFeature_dic[one_faFeature][1] + "," + faFeature_dic[one_faFeature][2] + "," + faFeature_dic[one_faFeature][3] + "," + RMB + "," + highMutationDensePercent + "," + one_readLen + "," + one_depth + "," + maxPrecisionSoftware + "," + maxRecallSoftware + "," + maxF_measureSoftware + "\n"      #输出为csv格式
