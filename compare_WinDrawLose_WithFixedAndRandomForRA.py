...
#计算WinDrawLose开始
Software_index = -1
for oneSoftware_fixed_RA_arr in fixed_RA_arr_allSoftware:
    Software_index = Software_index + 1
    win = 0
    draw = 0
    lose = 0
    for i in range(len(test_idx_arr)):
        if all_RA[i] > oneSoftware_fixed_RA_arr[i]:
            win = win + 1
        elif all_RA[i] == oneSoftware_fixed_RA_arr[i]:
            draw = draw + 1
        else:
            lose = lose + 1
    print fixed_arr[Software_index], ":", win, draw, lose

win = 0
draw = 0
lose = 0
for j in range(len(test_idx_arr)):
    if all_RA[j] > random_RA_arr[j]:
        win = win + 1
    elif all_RA[j] == random_RA_arr[j]:
        draw = draw + 1
    else:
        lose = lose + 1
print "random:", win, draw, lose
#计算WinDrawLose结束
